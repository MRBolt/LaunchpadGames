...
public class Launchpad {
	 ...
	 */
	public Launchpad(String deviceCue) throws MidiUnavailableException {
	 ...
	 * To be honest, you may aswell pass the {@link Launchpad} object to the class/method and
	...
	public void clearScreen() {
		for(int x = 1; x < 10; x++) {
			for(int y=1; y<10; y++) {
				this.sendNE(Launchpad.toMidi(x, y), 0);
			}
		}
	}
	...
	public void fill(int x1, int y1, int x2, int y2, int color) {
		for(int x = x1; x <= x2; x++) {
			for(int y=y1; y<=y2; y++) {
				this.sendNE(Launchpad.toMidi(x, y), color);
			}
		}
	}
